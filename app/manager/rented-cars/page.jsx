...
const page = () => {
    ...
    const loadCars = async () => {
        try {
            const response = await getRentedVehicles('car')
            setCars(response);
            if (response.length === 0) {
                toast.error('No cars are rented!');
            }
        } catch (error) {
            console.log(error);
            ...
            <div className="area w-full md:relative">
                <div className="py-12 padding-x border h-[100vh] overflow-y-scroll scrollbar-hide">
                    <div className="flex flex-col text-center w-full mb-10">
                        <h1 className="sm:text-3xl text-2xl font-medium title-font mb-2 text-gray-900">Rented cars</h1>
